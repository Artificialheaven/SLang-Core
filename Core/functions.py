...
@register.reg('换行', 0, '输出一个换行符')
def newline(parma: list, _dict: dict):
    return '\n'


@register.reg('调试输出', 1, '控制台输出调试文本')
def debug_print(parma: list, _dict: dict):
    print(parma[0])
...
def get_global_var(parma: list, _dict: dict):
...
def get_room_var(parma: list, _dict: dict):
...
def sleep_seconds(parma: list, _dict: dict):
...
def thread_run_room(parma: list, _dict: dict):
    ...
    threading.Thread(target=run, args=(parma[1], json.loads(parma[0]))).start()
...
def delect_str_in_line(parma: list, _dict: dict):
    le = parma[1].split('\n')
    del le[int(parma[0]) - 1]
    return '\n'.join(le)


@register.reg('取出行数', 2, '取出指定文本在来源的行号，失败返回-1。如果省略第二个参数，取出来源总行数')
def get_line_number(parma: list, _dict: dict):
    lines = parma[0].split('\n')
    if len(parma) == 2:
...
@register.reg('网页-下载', 4, '访问网页并将其下载到指定路径')
...
@register.reg('SQL-读取', 3, '使用变量中的sql执行sql查询，结果存入变量')
def read_sqlite3(parma: list, _dict: dict):
    cursor = _dict[parma[0]].cursor()
    cursor.execute(parma[1])
...
@register.reg('现行时间戳', 0, '获取当前Unix时间戳')
def current_timestamp(parma: list, _dict: dict):
    return str(int(time.time()))


@register.reg('uuid', 0, '获取随机的uuid')
def generate_uuid(parma: list, _dict: dict):
    return str(uuid.uuid1())


@register.reg('取反', 1, '若为真返回假，若为假，返回真')
def invert_boolean(parma: list, _dict: dict):
